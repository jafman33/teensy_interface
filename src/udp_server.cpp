...
{
  if (verbose_) {
    fmt::print("UDPServer shutting down...\n");
  }
  ...
  const std::string_view recvAddr)
{
  ...
  if (initialized_) {
    if (verbose_) {
      fmt::print("UDPServer already initialized. Resetting server...\n");
    }
    isRunning_ = false;
    recvThread_.join();
    ...
    if (verbose_) {
      fmt::print("UDPServer initializing...\n");
    }
  }
  ...
  if (verbose_) {
    if (sendAddr_.empty()) {
      fmt::print("Creating sending socket broadcasting on port: {}\n", sendPort_);
    } else {
      fmt::print("Creating sending socket for: {}:{}\n", sendAddr_, sendPort_);
    }
  }
  ...
  if (verbose_) {
    if (recvAddr_.empty()) {
      fmt::print("Creating receiving socket for port: {}\n", recvPort_);
    } else {
      fmt::print("Creating receiving socket for: {}:{}\n", recvAddr_, recvPort_);
    }
  }
  if (socket_recv_ < 0) {
  ...
  if (verbose_) {
    fmt::print("Starting receiving thread...\n");
  }
  ...
  if (verbose_) {
    fmt::print("UDPServer initialized.\n");
  }
}
